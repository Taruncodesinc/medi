...
export const register: RequestHandler = async (req, res) => {
  ...
  const user = await User.create({ name, email, passwordHash, role, phone, verificationCode: code, verificationCodeExpires: expires, isVerified: false });
...
export const verifyCode: RequestHandler = async (req, res) => {
  ...
  if (String(user.verificationCode) !== code || !user.verificationCodeExpires || user.verificationCodeExpires < new Date()) {
...
export const resetPassword: RequestHandler = async (req, res) => {
  ...
  const user = await User.findOne({ email });
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (String(user.resetCode) !== code || !user.resetCodeExpires || user.resetCodeExpires < new Date()) {
    return res.status(400).json({ error: 'Invalid or expired code' });
  }
